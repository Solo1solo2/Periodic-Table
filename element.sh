   ...
   then
    ...
    if [[ -z $ELEMENT ]]
     then
      ...
      echo $ELEMENT | while IFS=\| read ATOMIC_NUMBER ATOMIC_MASS MTP BLP SYM NAME TYPE
       do
         echo "The element with atomic number $ATOMIC_NUMBER is $NAME ($SYM). It's a $TYPE, with a mass of $ATOMIC_MASS amu. $NAME has a melting point of $MTP celsius and a boiling point of $BLP celsius."
        done
        fi
    else
